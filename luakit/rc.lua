...
completion.completers.opentabs = {
  header = { "URI", "Title" },
  func = function (buf)
    local ret = {}
    for i, view in ipairs(window.tabs.children) do  -- enumerate open tabs :contentReference[oaicite:3]{index=3}
      local uri   = view.uri   or ""
      local title = view.title or ""
      local label = (title ~= "" and title or uri)
      if buf == "" or label:lower():find(buf:lower(),1,true) then
        table.insert(ret, { uri, title, format = uri })
      end
    end
    return ret
  end,
...
table.insert(completion.order, 1, function (w, text)
  local cmd, rest = text:match("^(%S+)%s*(.*)$")
  if cmd ~= "tabfocus" then return {} end
  return completion.completers.opentabs.func(rest)
end)
